(function() {
    ...
    const styles = `
        /* Your existing CSS */
        #chatbot-widget { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; position: fixed; z-index: 999999; ${config.position}: 20px; bottom: 20px; }
        .chat-toggle { width: 60px; height: 60px; background: linear-gradient(135deg, ${config.primaryColor}, ${adjustColor(config.primaryColor, -20)}); border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 8px 25px rgba(0,0,0,0.15); transition: all 0.3s ease; border: none; color: white; }
        .chat-toggle:hover { transform: scale(1.1); box-shadow: 0 12px 35px rgba(0,0,0,0.2); }
        .chat-toggle svg { width: 28px; height: 28px; fill: white; transition: transform 0.3s ease; }
        .chat-toggle.active svg { transform: rotate(45deg); }
        .chat-container { position: absolute; bottom: 80px; ${config.position}: 0; width: 380px; height: 520px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px); border-radius: 20px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.15); transform: translateY(20px) scale(0.95); opacity: 0; visibility: hidden; transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid rgba(255, 255, 255, 0.2); overflow: hidden; display: flex; flex-direction: column; }
        .chat-container.active { transform: translateY(0) scale(1); opacity: 1; visibility: visible; }
        .chat-header { background: linear-gradient(135deg, ${config.primaryColor}, ${adjustColor(config.primaryColor, -20)}); color: white; padding: 20px; display: flex; align-items: center; gap: 12px; }
        ...
        .chat-info h3 { font-size: 16px; font-weight: 600; margin: 0 0 2px 0; }
        .chat-status { font-size: 12px; opacity: 0.9; display: flex; align-items: center; gap: 6px; margin: 0; }
        ...
        .message.bot { display: flex; justify-content: flex-start; flex-direction: column; align-items: flex-start; }
        .message.system { text-align: center; }
        .message-bubble { max-width: 75%; padding: 12px 16px; border-radius: 18px; font-size: 14px; line-height: 1.4; word-wrap: break-word; }
        .message.user .message-bubble { background: linear-gradient(135deg, ${config.primaryColor}, ${adjustColor(config.primaryColor, -20)}); color: white; border-bottom-right-radius: 4px; }
        .message.bot .message-bubble { background: #f1f5f9; color: #334155; border-bottom-left-radius: 4px; border: 1px solid rgba(0, 0, 0, 0.05); }
        .message.system .message-bubble { background: ${config.primaryColor}20; color: ${config.primaryColor}; font-size: 12px; padding: 8px 12px; border-radius: 12px; display: inline-block; }
        .feedback-container { margin-top: 8px; display: flex; gap: 8px; align-items: center; }
        .feedback-button { 
            padding: 8px 12px; 
            font-size: 16px; 
            border: none;
        ...
        .chat-input { padding: 20px; background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(10px); border-top: 1px solid rgba(0, 0, 0, 0.06); }
        .input-group { display: flex; gap: 10px; align-items: flex-end; }
        .message-input { flex: 1; padding: 12px 16px; border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 20px; font-size: 14px; resize: none; min-height: 20px; max-height: 80px; background: white; transition: all 0.2s ease; }
        .message-input:focus { outline: none; border-color: ${config.primaryColor}; box-shadow: 0 0 0 3px ${config.primaryColor}20; }
        .send-button { width: 44px; height: 44px; background: linear-gradient(135deg, ${config.primaryColor}, ${adjustColor(config.primaryColor, -20)}); border: none; border-radius: 50%; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; }
        .send-button:hover { transform: scale(1.05); box-shadow: 0 4px 15px ${config.primaryColor}40; }
        .send-button svg { width: 18px; height: 18px; fill: white; }
        .messages::-webkit-scrollbar { width: 4px; }
        .messages::-webkit-scrollbar-thumb { background: ${config.primaryColor}40; border-radius: 2px; }
        @media (max-width: 480px) { .chat-container { width: calc(100vw - 40px); height: 480px; } }
    `;
        ...
        <div id="chatbot-widget">
            ...
            <div class="chat-container" id="chat-container">
                <div class="chat-header">
                    <div class="chat-avatar"><svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2ZM21 9V7L15 1L9 7V9C9 10.1 9.9 11 11 11V16L13 18L15 16V11C16.1 11 17 10.1 17 9H21Z"/></svg></div>
                    <div class="chat-info"><h3>${config.botName}</h3><div class="chat-status"><div class="status-dot"></div>Online</div></div>
                </div>
                <div class="messages" id="messages"></div>
                <div class="chat-input"><div class="input-group"><input type="text" class="message-input" id="message-input" placeholder="Type your message..." onkeypress="ChatbotWidget.handleKeyPress(event)"><button class="send-button" onclick="ChatbotWidget.sendMessage()"><svg viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></button></div></div>
            </div>
        </div>
    `;
    ...
    function initWidget() {
        ...
        window.ChatbotWidget = {
            ...
            handleKeyPress(event) { if (event.key === 'Enter') this.sendMessage(); },
            ...
            updateLastBotMessage(content, qid = null) {
                ...
                lastMessage.querySelector('.message-bubble').innerHTML = content;

                // Add feedback buttons only if not present
            ...
            async sendMessage() {
                ...
                // 👉 Load previous chat history BEFORE adding the new message
                const saved = localStorage.getItem('chatHistory');
                let lastThree = [];
                if (saved) {
                    const messages = JSON.parse(saved);
                    ...
                    for (let i = 0; i < messages.length - 1; i++) {
                        if (messages[i].type === "user" && messages[i + 1]?.type === "bot") {
                            ...
                            // ❌ Skip placeholder or empty bot messages
                            if (ans && ans !== "💭 ...") {
                                qaPairs.push({
                    ...
                    lastThree = qaPairs.slice(-3); // only last 3 completed Q&A
                }


                // Now add the user message after extracting history
                ...
                try {
                    const response = await fetch(`${config.apiUrl}/chat`, {
                        ...
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ 
                            question: message,   // current question
                            history: lastThree   // only previous 3 Q&A pairs
                        })
                    });
        ...
        // Close on outside click but prevent closing when clicking on feedback buttons
        document.addEventListener('click', function(event) {
            const chatContainer = document.getElementById('chat-container');
            const chatWidget = document.getElementById('chatbot-widget');
            
            // Check if clicked element is a feedback button or its parent
            if (event.target.closest('.feedback-container')) {
                return; // Don't close widget when clicking feedback
            }
